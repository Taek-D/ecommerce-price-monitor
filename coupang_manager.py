...
_log_api = logging.getLogger("musinsa_bot.coupang.api")
_log_order = logging.getLogger("musinsa_bot.coupang.order")
_log_ship = logging.getLogger("musinsa_bot.coupang.shipping")
_log_sync = logging.getLogger("musinsa_bot.coupang.sync")
_log_sourcing = logging.getLogger("musinsa_bot.coupang.sourcing")
_log_stock = logging.getLogger("musinsa_bot.coupang.stock")
_log_settlement = logging.getLogger("musinsa_bot.coupang.settlement")
_log_sms = logging.getLogger("musinsa_bot.coupang.sms")
...
def _log_api_error(method: str, response: httpx.Response) -> None:
    ...
    if (
        ...
        _log_api.error(
            "Hint: endpoint path/version or HTTP method may be wrong for this API."
        )
...
def _build_price_change_embed(change: dict) -> dict:
    ...
    fields = [
        {
            "name": "상품명",
            "value": _short_text(change.get("name", ""), 180),
            "inline": False,
        },
        {
            "name": "기준 판매가",
...
def _normalize_vendor_item_id(raw_value: str | int | None) -> str | None:
    ...
    if len(digits) < _VENDOR_ITEM_ID_MIN_LENGTH:
        return None
...
def _parse_vendor_item_ids(raw_value: str) -> list[str]:
    ...
    seen: set[str] = set()
    for token in tokens:
        vid = _normalize_vendor_item_id(token)
...
def _load_sourcing_min_price_by_vid() -> dict[str, int]:
    ...
    try:
        ...
        _log_order.error(f"소싱목록 조회 실패: {e}")
        return {}
...
def _flush_sheet_cell_updates(
    ...
    for i in range(0, len(items), chunk_size):
        ...
        try:
            ...
            _log_sheet.error(f"batch_update 실패 (chunk={len(chunk)}): {e}")
            for rng, val in chunk:
                try:
                    ws.update(rng, [[val]], value_input_option="USER_ENTERED")
                except Exception as inner:
                    _log_sheet.error(f"셀 업데이트 실패 ({rng}): {inner}")
...
async def get_orders_by_status(status: str, days: int = 7) -> list[dict]:
    ...
    try:
        ...
        _log_order.info(f"{status} 조회 → {len(all_orders)}건")
        return all_orders
    except Exception as e:
        _log_order.error(f"주문 조회 실패 (status={status}): {e}")
        return []
...
async def _order_exists_in_coupang(order_id: str) -> bool | None:
    ...
    try:
        ...
        _log_order.error(f"주문 존재확인 실패(orderId={oid}): HTTP {status_code}")
        return None
    except Exception as e:
        _log_order.error(f"주문 존재확인 예외(orderId={oid}): {e}")
        return None
...
async def confirm_order(order_id: str, shipment_box_id: str) -> bool:
    ...
    if not box_id:
        ...
        if not box_id:
            _log_order.error(f"발주확인 실패 → shipmentBoxId 없음 (orderId={order_id})")
            return False
    if not box_id.isdigit():
        _log_order.error(
            f"발주확인 실패 → shipmentBoxId 형식오류 (orderId={order_id}, shipmentBoxId={box_id})"
        )
        return False
    ...
    try:
        ...
        if (
            ...
            _log_order.info(
                f"발주확인 완료 → 주문ID: {order_id} shipmentBoxId={box_id}"
            )
            return True
        ...
        _log_order.error(
            f"발주확인 실패 → 주문ID: {order_id} shipmentBoxId={box_id} | {reason}"
        )
        return False
    except Exception as e:
        _log_order.error(f"발주확인 예외: {e}")
        return False
...
async def get_order_sheet_ids() -> set[str]:
    ...
    try:
        ...
        _log_sheet.error(f"주문시트 조회 실패: {e}")
        return set()
...
async def process_new_orders():
    ...
    """
    _log_order.info(f"주문 동기화 시작... ({_now_kst_str()})")
    ...
    if not all_orders:
        _log_order.info("조회된 주문 없음 (결제완료 + 상품준비중)")
        return
    ...
    try:
        ...
        _log_sheet.error(f"주문시트 열기 실패: {e}")
        return
    ...
    if min_price_by_vid:
        _log_order.info(
            f"소싱 최소판매금액 매핑 로드: {len(min_price_by_vid)}개 vendorItemId"
        )
    else:
        _log_order.warning(
    ...
    for order in accept_orders:
        ...
        if reason in {
            ...
            if missing_key not in _price_guard_warned_missing:
                ...
                _log_order.warning(
                    f"[PriceGuard] 경고 후 진행: {reason_label} | "
        ...
        if guard["blocked"]:
            ...
            if low_key not in _price_guard_warned_low:
                ...
                _log_order.error(
                    f"[PriceGuard] 가격미달 보류: orderId={order_id} | "
        ...
        if order_id in processed_ids:
            ...
            if needs_status_retry:
                ...
                if confirmed:
                    ...
                    updated_count += 1
                    _log_order.warning(
                        f"발주확인 실패 — 시트 상태를 결제완료로 유지: {order_id}"
                    )
    ...
    for order in instruct_orders:
        ...
        _log_order.info(
            f"[상품준비중] {order_id} | {_order_item_name(item)} — 시트 추가"
        )
        sms_sent = False
        if phone:
            sms_sent = await send_order_privacy_sms(phone)
    ...
    _log_order.info(f"완료 — 신규 추가 {new_count}건, 상태갱신 {updated_count}건")
...
async def sync_delivery_status_to_sheet(days: int = 60) -> None:
    ...
    - FINAL_DELIVERY -> 배송완료
    """
    _log_order.info(f"배송상태 동기화 시작... ({_now_kst_str()})")

    try:
        ws = _open_coupang_sheet(COUPANG_ORDER_SHEET)
        rows = ws.get_all_values()
    except Exception as e:
        _log_order.error(f"배송상태 동기화 실패(시트 열기): {e}")
        return
    ...
    _log_order.info(f"배송상태 조회 범위: 최근 {lookback_days}일")
    ...
    if not latest_status_by_order_id:
        _log_order.info("배송상태 동기화 대상 없음 (상태갱신 스킵, 삭제검사 진행)")
    ...
    if delete_candidates:
        _log_order.info(
            f"쿠팡 미존재 주문 감지 — 시트 삭제 대상 {len(delete_candidates)}건"
        )
    ...
    for row_idx, order_id in sorted(
        ...
        try:
            ...
            delete_failed += 1
            _log_order.error(
                f"시트 주문 삭제 실패 (row={row_idx}, orderId={order_id}): {e}"
            )
...
SOURCING_SIBLING_SCORE_WINDOW = _env_int("SOURCING_SIBLING_SCORE_WINDOW", 2)
...
def _name_count_set(name: str) -> set[str]:
    normalized = _normalize_product_name(name)
    if not normalized:
        return set()
    return set(re.findall(r"(\d+)개", normalized))


def _build_product_name_vid_index(
    ...
    return name_to_vids, name_display


def _match_sourcing_vendor_item_ids(
    sourcing_name: str,
    name_to_vids: dict[str, set[str]],
) -> tuple[list[str], str, int, str]:
    if not sourcing_name or not name_to_vids:
        return [], "", 0, ""
    ...
    if exact_keys:
        ...
        if best_score < SOURCING_MATCH_THRESHOLD or score_gap < SOURCING_MATCH_MIN_GAP:
            return [], "", 0, ""
    ...
    matched_vids = set(name_to_vids.get(matched_key, set()))
    if not matched_vids:
        return [], "", 0, ""

    source_tokens = _name_token_set(sourcing_name)
    source_counts = _name_count_set(sourcing_name)
    matched_tokens = _name_token_set(matched_key)
    matched_counts = _name_count_set(matched_key)
    score_floor = max(
        SOURCING_MATCH_THRESHOLD, matched_score - SOURCING_SIBLING_SCORE_WINDOW
    )

    for candidate_key, candidate_vids in name_to_vids.items():
        if candidate_key == matched_key:
            continue

        candidate_counts = _name_count_set(candidate_key)
        if source_counts:
            if candidate_counts != source_counts:
                continue
        else:
            if matched_counts:
                if candidate_counts != matched_counts:
                    continue
            elif any(int(value) > 1 for value in candidate_counts):
                continue

        candidate_tokens = _name_token_set(candidate_key)
        if not candidate_tokens:
            continue

        source_score = max(
            _fuzzy_name_score(source_key, candidate_key) for source_key in source_keys
        )
        sibling_score = _fuzzy_name_score(matched_key, candidate_key)
        overlap_with_best = len(matched_tokens.intersection(candidate_tokens))
        overlap_with_source = len(source_tokens.intersection(candidate_tokens))
        smaller_token_count = max(1, min(len(matched_tokens), len(candidate_tokens)))
        overlap_ratio = overlap_with_best / smaller_token_count

        if (
            source_score >= score_floor
            and sibling_score >= SOURCING_SIBLING_MATCH_MIN
            and overlap_with_best >= max(3, len(matched_tokens) - 2)
            and overlap_ratio >= 0.7
            and overlap_with_source >= max(2, len(source_tokens) - 2)
        ):
            matched_vids.update(candidate_vids)

    return (
        sorted(matched_vids, key=lambda x: int(x) if x.isdigit() else x),
        matched_key,
        matched_score,
        match_mode,
    )
...
def _pick_representative_vendor_item_id(
    matched_key: str,
    matched_vids: list[str],
    name_to_vids: dict[str, set[str]],
    preferred_vid: str | None = None,
) -> str:
    if preferred_vid and preferred_vid in matched_vids:
        return preferred_vid

    primary_vids = sorted(
        name_to_vids.get(matched_key, set()),
        key=lambda x: int(x) if x.isdigit() else x,
    )
    if preferred_vid and preferred_vid in primary_vids:
        return preferred_vid
    if primary_vids:
        return primary_vids[0]
    return matched_vids[0] if matched_vids else ""
...
async def _fetch_product_sheet_snapshot_from_api() -> list[dict]:
    ...
    while True:
        ...
        try:
            ...
            _log_product.error(f"상품 목록 조회 실패: {e}")
            break
        ...
        for product in products:
            ...
            try:
                ...
                _log_product.error(
                    f"상품 상세 조회 실패: sellerProductId={seller_product_id} | {e}"
                )
...
async def refresh_product_sheet_from_api(force: bool = False) -> bool:
    ...
    try:
        ...
        _log_product.error(f"시트 열기 실패: {e}")
        return False
    ...
    _log_product.info(f"쿠팡상품관리 최신화 시작... ({_now_kst_str()})")
    snapshot = await _fetch_product_sheet_snapshot_from_api()
    if not snapshot:
        _log_product.info("최신화 대상 상품이 없습니다")
        return False
    ...
    try:
        ...
        _log_product.error(f"시트 쓰기 실패: {e}")
        return False
    ...
    _log_product.info(f"{len(snapshot)}개 옵션 최신화 완료")
    return True
...
async def update_sale_price(vendor_item_id: str, new_price: int) -> bool:
    ...
    if new_price % 10 != 0:
        _log_sync.error(
            f"판매가 변경 실패 → 10원 단위만 허용 (vendorItemId={vendor_item_id}, price={new_price})"
        )
        return False
...
async def update_stock(vendor_item_id: str, quantity: int) -> bool:
    ...
    try:
        ...
        code = str(result.get("code", ""))
        if code in ("SUCCESS", "200"):
            _log_stock.info(f"재고 변경 → vendorItemId={vendor_item_id} | {quantity}개")
            return True
        else:
            _log_stock.error(f"재고 변경 실패 → {result}")
            return False
    except Exception as e:
        _log_stock.error(f"재고 변경 예외: {e}")
        return False
...
async def update_sale_status(vendor_item_id: str, on_sale: bool) -> bool:
    ...
    try:
        ...
        if code in ("SUCCESS", "200"):
            _log_sync.info(f"판매상태 변경 → {vendor_item_id} | {status_str}")
            return True
        else:
            _log_sync.error(f"판매상태 변경 실패 → {result}")
            return False
    except Exception as e:
        _log_sync.error(f"판매상태 변경 예외: {e}")
        return False
...
async def sync_products_from_sheet():
    ...
    """
    global _sync_baseline_initialized
    _log_sync.info(f"상품 동기화 시작... ({_now_kst_str()})")

    try:
        ws = _open_coupang_sheet(COUPANG_PRODUCT_SHEET)
        rows = ws.get_all_values()
    except Exception as e:
        _log_sync.error(f"시트 열기 실패: {e}")
        return

    data_rows = rows[PRODUCT_START_ROW - 1 :]  # 헤더 제외
    is_first_sync = not _sync_baseline_initialized
    if is_first_sync:
        _log_sync.info("최초 동기화: 기준 상태만 적재하고 API 반영은 생략")
...
async def sync_price_from_sourcing():
    ...
    try:
        ...
        col_price_vid = ws.col_values(SOURCING_COL_PRICE_VID)  # P
    except Exception as e:
        _log_sourcing.error(f"시트 열기 실패: {e}")
        return
    ...
    try:
        ...
        _log_sourcing.error(f"쿠팡상품관리 매핑 인덱스 생성 실패: {e}")
    ...
    for i in range(SOURCING_DATA_START, max_row + 1):
        ...
        price_vid_cell = (
            col_price_vid[i - 1].strip() if i - 1 < len(col_price_vid) else ""
        )

        if (
            not name_cell
            and not buy_price_cell
            and not price_cell
            and not vid_cell
            and not price_vid_cell
        ):
            continue
        ...
        if (
            ...
            if len(augmented_ids) > len(vendor_item_ids):
                ...
                _log_sourcing.info(
                    f"vendorItemId 보강 매핑 → row={i} '{name_cell}' "
                    f"| {old_count}개 → {len(vendor_item_ids)}개"
                )
            elif len(vendor_item_ids) == 1:
                _log_sourcing.info(
                    f"vendorItemId 보강 불가 유지 → row={i} '{name_cell}' "
                    f"| O열='{vid_cell}'"
                )
        ...
        if not vendor_item_ids:
            if is_soldout_row:
                _log_sourcing.warning(
                    f"vendorItemId 매핑 없음 스킵 → row={i} name='{name_cell}' O열='{vid_cell}'"
                )
            continue
        ...
        if not price_vendor_item_ids:
            if vendor_item_ids:
                _log_sourcing.warning(
                    f"가격동기화 vendorItemId 없음 스킵 → row={i} "
        ...
        for price_vendor_item_id in price_vendor_item_ids:
            ...
            if current_sale_price is None:
                ...
                _log_sourcing.warning(
                    f"현재 판매가 미확인 스킵 → vendorItemId={price_vendor_item_id}"
                )
        ...
        if skipped_unknown_ids or failed_ids:
            _log_sourcing.warning(
                f"일부 대상 재시도 예정으로 상태 갱신 보류 → row={i} "
...
async def auto_match_sourcing_vendor_item_ids():
    ...
    3) 거의 동일한 형제 상품은 보수적으로 함께 묶는다.
    """
    _log_sourcing.info(f"자동 매칭 확인... ({_now_kst_str()})")
    ...
    try:
        ...
        _log_sourcing.error(f"O/P열 헤더 보정 실패: {e}")

    name_to_vids, name_display = _build_product_name_vid_index(product_rows)

    if not name_to_vids:
        _log_sourcing.info("쿠팡상품관리에 매칭 대상 상품이 없습니다")
        return
    ...
    for i, row in enumerate(
        ...
        existing_price_vid_raw = (
            (row[SOURCING_COL_PRICE_VID - 1] or "").strip()
        ...
        desired_price_vid = _pick_representative_vendor_item_id(
            ...
            matched_vids,
            name_to_vids,
            preferred_vid=existing_price_vid,
        )
        if not desired_price_vid:
            continue

        update_vids = False
        if not existing_vids:
            update_vids = True
        elif not set(existing_vids).issuperset(set(matched_vids)):
        ...
        if update_vids and existing_vid and not existing_vids:
            repaired_count += 1
        if update_vids and len(matched_vids) > max(len(existing_vids), 1):
            augmented_count += 1
        if update_price_vid:
        ...
        updates.append(
            {
                "row": i,
                "vids": ",".join(matched_vids),
                "price_vid": desired_price_vid,
                "previous_vids": existing_vid,
                "previous_price_vid": existing_price_vid_raw,
                "source_name": sourcing_name,
                "matched_name": name_display.get(matched_key, matched_key),
                "score": matched_score,
                "match_mode": match_mode,
                "update_vids": update_vids,
                "update_price_vid": update_price_vid,
            }
        )

    if not updates:
        _log_sourcing.info("신규 매칭 없음")
        return
    ...
    try:
        ...
        for u in updates:
            ...
            if u["update_price_vid"]:
                batch_body.append(
                    {
                        "range": gspread.utils.rowcol_to_a1(
                            u["row"], SOURCING_COL_PRICE_VID
                        ),
                        "values": [[u["price_vid"]]],
                    }
                )
        ...
        _log_sourcing.error(f"O/P열 일괄 업데이트 실패: {e}")
        return

    _log_sourcing.info(
        f"{len(updates)}건 자동 매칭 반영 "
        f"(exact={exact_count}, fuzzy={fuzzy_count}, repaired={repaired_count}, "
        f"augmented={augmented_count}, representative={representative_count})"
    )
    ...
    if COUPANG_ORDER_WEBHOOK:
        ...
        embeds = [
            {
                ...
                "fields": [
                    ...
                    {
                        "name": "기존값 보정",
                        "value": f"{repaired_count}건",
                        "inline": True,
                    },
                    {
                        "name": "다중 매칭",
                        "value": f"{augmented_count}건",
                        "inline": True,
                    },
                    {
                        "name": "대표값 보정",
                        "value": f"{representative_count}건",
                        "inline": True,
                    },
                ],
            }
        ...
        await post_webhook(
            COUPANG_ORDER_WEBHOOK, "소싱목록 O/P열 자동 매칭 완료", embeds=embeds
        )
...
async def get_shipment_box_id(order_id: str) -> tuple[str, str]:
    ...
    try:
        ...
        _log_ship.warning(f"orderId 단건조회(v5) 실패 → {e}")
    ...
    try:
        ...
        _log_ship.error(f"orderId={order_id} → shipmentBoxId 조회 실패")
        return "", ""
    except Exception as e:
        _log_ship.error(f"shipmentBoxId 조회 예외: {e}")
        return "", ""
...
async def ship_order_api(
    ...
    if not order_item_id:
        _log_ship.warning("shipmentBoxId 누락 — 송장등록 스킵")
        return False
    if not vendor_item_id:
        _log_ship.warning(
            f"vendorItemId 누락 — orderId={order_id} shipmentBoxId={order_item_id}"
        )
        return False
    if not str(order_id or "").isdigit():
        _log_ship.error(f"orderId 형식오류 — orderId={order_id}")
        return False
    if not str(vendor_item_id or "").isdigit():
        _log_ship.error(f"vendorItemId 형식오류 — vendorItemId={vendor_item_id}")
        return False
    ...
    try:
        ...
        if code in ("200", "SUCCESS"):
            ...
            if response_list and response_list[0].get("succeed"):
                _log_ship.info(
                    f"송장등록 완료 → shipmentBoxId={order_item_id} | 송장={invoice_number} ({carrier_code})"
                )
                return True
            else:
                msg = response_list[0].get("resultMessage", "") if response_list else ""
                _log_ship.error(f"송장등록 실패 → {msg}")
                return False
        else:
            _log_ship.error(f"API 오류 → {result}")
            return False
    except Exception as e:
        _log_ship.error(f"송장등록 예외: {e}")
        return False
...
async def process_shipping():
    ...
    """
    _log_ship.info(f"배송처리 대기 주문 확인... ({_now_kst_str()})")

    try:
        ws = _open_coupang_sheet(COUPANG_ORDER_SHEET)
        rows = ws.get_all_values()
    except Exception as e:
        _log_ship.error(f"시트 열기 실패: {e}")
        return
    ...
    for i, row in enumerate(data_rows, start=ORDER_START_ROW):
        ...
        if carrier_code not in VALID_CARRIER_CODES:
            _log_ship.warning(
                f"택배사코드 오류 → orderId={order_id} | 입력='{carrier}' | 변환='{carrier_code}'"
            )
            continue

        if carrier_code != carrier.upper():
            _log_ship.info(f"택배사코드 보정: '{carrier}' → '{carrier_code}'")
        ...
        vendor_item_id = ""
        if not order_item_id:
            _log_ship.info(f"J열 없음 → orderId={order_id} 로 shipmentBoxId 조회")
            order_item_id, vendor_item_id = await get_shipment_box_id(order_id)
            if not order_item_id:
                _log_ship.warning(f"{order_id} shipmentBoxId 조회 실패 — 스킵")
                continue
        else:
            # J열이 채워진 케이스도 vendorItemId가 필요하므로 보완 조회
            _, vendor_item_id = await get_shipment_box_id(order_id)
            if not vendor_item_id:
                _log_ship.warning(f"{order_id} vendorItemId 조회 실패 — 스킵")
                continue
    ...
    if shipped_count == 0:
        _log_ship.info("처리할 배송 없음")
    else:
        _log_ship.info(f"총 {shipped_count}건 배송처리 완료")
...
async def get_vendor_item_stock(vendor_item_id: str) -> dict:
    ...
    normalized_vendor_item_id = _normalize_vendor_item_id(vendor_item_id)
    if not normalized_vendor_item_id:
        if str(vendor_item_id or "").strip():
            _log_stock.warning(
                f"vendorItemId 형식오류 스킵: {str(vendor_item_id).strip()}"
            )
        return {}
    vendor_item_id = normalized_vendor_item_id
    ...
    try:
        ...
        if getattr(e.response, "status_code", None) == 404:
            _log_stock.warning(
                f"vendorItemId={vendor_item_id} 미존재/비공개(404) - 시트 확인 필요"
            )
            return {}
        _log_stock.error(f"vendorItemId={vendor_item_id} 조회 실패: {e}")
        return {}
    except Exception as e:
        _log_stock.error(f"vendorItemId={vendor_item_id} 조회 실패: {e}")
        return {}
...
async def auto_stock_out_check():
    ...
    """
    _log_stock.info(f"실재고 자동 점검 시작... ({_now_kst_str()})")

    try:
        ws = _open_coupang_sheet(COUPANG_PRODUCT_SHEET)
        rows = ws.get_all_values()
    except Exception as e:
        _log_stock.error(f"시트 열기 실패: {e}")
        return
    ...
    for i, row in enumerate(data_rows, start=PRODUCT_START_ROW):
        ...
        vendor_item_id = _normalize_vendor_item_id(row[COL_VENDOR_ITEM_ID - 1])
        if not vendor_item_id:
            continue
        product_name = (
            row[COL_PRODUCT_NAME - 1].strip() if len(row) > COL_PRODUCT_NAME - 1 else ""
        ...
        if real_stock == 0 and on_sale:
            # ── 재고 0 이고 판매중 → 품절처리 ──
            _log_stock.info(
                f"품절 감지 → {product_name} (vendorItemId={vendor_item_id})"
            )
            success = await update_sale_status(vendor_item_id, False)
            if success:
            ...
            # ── 재고 복구 이고 품절상태 → 판매재개 ──
            _log_stock.info(f"재고 복구 → {product_name} ({real_stock}개)")
            ...
            _log_stock.info(
                f"판매중지 유지 → {product_name} (재고 {real_stock}개, 판매재개 생략)"
            )
            _stock_status[vendor_item_id] = False
        else:
...
async def update_settlement():
    ...
    - 마지막 갱신 시각 기록
    """
    _log_settlement.info(f"정산 집계 시작... ({_now_kst_str()})")
    ...
    try:
        ...
        _log_settlement.error(f"주문시트 열기 실패: {e}")
        return
    ...
    try:
        ...
        _log_settlement.info(f"'{SETTLEMENT_SHEET}' 탭 자동 생성")
    ...
    try:
        ...
        _log_settlement.info(
            f"정산집계 갱신 완료 | 월별 {len(monthly)}개월 / 상품 {len(by_product)}종"
        )
    except Exception as e:
        _log_settlement.error(f"시트 기록 실패: {e}")
        return
...
if __name__ == "__main__":
    ...
    async def _test():
        ...
        if test_phone:
            ...
            _log_api.info(f"SMS 결과: {result}")
        else:
            _log_api.info("COUPANG_TEST_PHONE 미설정 → SMS 테스트 스킵")

        if os.getenv("COUPANG_TEST_SYNC", "0").strip() == "1":
            await coupang_sync_job()
            _log_api.info("COUPANG_TEST_SYNC=1 → 상품 동기화 테스트 실행")
        else:
            _log_api.info("COUPANG_TEST_SYNC=1 설정 시 상품 동기화 테스트 실행")

    if os.getenv("COUPANG_RUN_SELF_TEST", "0").strip() == "1":
        asyncio.run(_test())
    else:
        _log_api.info("Self-test disabled. Set COUPANG_RUN_SELF_TEST=1 to run.")
