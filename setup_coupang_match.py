...
def _make_sig(method, path, query=""):
    ...
    sig = hmac.new(
        COUPANG_SECRET_KEY.encode(), msg.encode(), hashlib.sha256
    ).hexdigest()
    return {
        "Authorization": (
    ...
    while True:
        ...
        try:
            async with httpx.AsyncClient(timeout=30) as client:
                r = await client.get(
                    COUPANG_BASE_URL + path + "?" + query, headers=headers
                )
                data = r.json()
        ...
        print(
            f"  [DEBUG] 응답: code={data.get('code')}, data길이={len(data.get('data', []))}, keys={list(data.keys())}"
        )
        code = data.get("code", "")
        if code != "SUCCESS":
            print(f"  ❌ 쿠팡 API 오류: {data.get('message', data)}")
            print(
                "  ⚠️  API 키 발급 후 24시간이 지나지 않았다면 내일 다시 시도해주세요."
            )
            break
        ...
        for product in products:
            ...
            try:
                async with httpx.AsyncClient(timeout=30) as client:
                    dr = await client.get(
                        COUPANG_BASE_URL + detail_path, headers=detail_headers
                    )
                    detail = dr.json()
            except Exception as e:
            ...
            if detail.get("code") != "SUCCESS":
                print(
                    f"  ❌ 상세 API 오류 ({seller_product_id}): {detail.get('message')}"
                )
                continue
            ...
            for item in items:
                ...
                try:
                    async with httpx.AsyncClient(timeout=30) as client:
                        ir = await client.get(
                            COUPANG_BASE_URL + inv_path, headers=inv_headers
                        )
                        inv = ir.json()
                except Exception as e:
                ...
                all_items.append(
                    {
                        ...
                        "salePrice": inv_data.get(
                            "salePrice", inv_data.get("price", item.get("salePrice", 0))
                        ),
                        "stock": inv_data.get(
                            "amountInStock",
                            inv_data.get("quantity", item.get("maximumBuyCount", 0)),
                        ),
                        "status": item_status,
                    }
                )
...
def match_sourcing_to_coupang(gc, coupang_items):
    ...
    for i, row in enumerate(data_rows, start=SOURCING_DATA_START):
        if (
            not row
            or len(row) < SOURCING_COL_NAME
            or not row[SOURCING_COL_NAME - 1].strip()
        ):
            continue
    ...
    for u in updates:
        print(
            f"  [{u['score']:3d}점] '{u['sourcing_name']}' → '{u['coupang_name']}' | IDs: {u['vids']}"
        )
